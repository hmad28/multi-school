...
import type { ClassOption, Student } from '@/types/domain';
import { Head, Link, useForm, usePage } from '@inertiajs/vue3';

const props = defineProps<{ student: Student; classes: ClassOption[] }>();
const page = usePage();
const school = page.props.school as { slug: string };
const form = useForm({ ...props.student });

function submit(): void {
    form.patch(route('tenant.students.update', { tenant: school.slug, student: props.student.id }));
}
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header><div><p class="page-kicker">Master Data</p><h1 class="text-2xl font-bold text-ink dark:text-white">Edit Siswa</h1><p class="text-sm text-slate-500">Perbarui biodata siswa.</p></div></template>
