...
import type { AcademicLevel, SchoolClass, Teacher } from '@/types/domain';
import { Head, Link, useForm, usePage } from '@inertiajs/vue3';

const props = defineProps<{ classData: Partial<SchoolClass>; levels: AcademicLevel[]; teachers: Pick<Teacher, 'id' | 'full_name'>[] }>();
const page = usePage();
const school = page.props.school as { slug: string };
const form = useForm({ ...props.classData });

function submit(): void { form.patch(route('tenant.classes.update', { tenant: school.slug, class: props.classData.id })); }
</script>

<template>
    <Head title="Edit Kelas" />
    <AuthenticatedLayout>
        <template #header><div><p class="page-kicker">Master Data</p><h1 class="text-2xl font-bold text-ink dark:text-white">Edit Kelas</h1><p class="text-sm text-slate-500">Perbarui data kelas.</p></div></template>
        <form class="app-card max-w-3xl space-y-5 p-6" @submit.prevent="submit">
            <div class="grid gap-5 md:grid-cols-2">
                <label class="space-y-2"><span class="text-sm font-semibold">Tingkat</span><select v-model="form.academic_level_id" class="app-input w-full"><option v-for="level in levels" :key="level.id" :value="level.id">{{ level.name }}</option></select><span class="text-sm text-rose-600">{{ form.errors.academic_level_id }}</span></label>
                <label class="space-y-2"><span class="text-sm font-semibold">Nama kelas</span><input v-model="form.name" class="app-input w-full" /><span class="text-sm text-rose-600">{{ form.errors.name }}</span></label>
