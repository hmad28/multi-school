...
const props = defineProps<{ teacher: { id: string; nip?: string | null; full_name: string; position: string; phone?: string | null; status: string; can_input_teacher_attendance: boolean } }>();
const page = usePage();
const school = page.props.school as { slug: string };
const form = useForm({ ...props.teacher });

function submit(): void {
    form.patch(route('tenant.teachers.update', { tenant: school.slug, teacher: props.teacher.id }));
}
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header><div><p class="page-kicker">Master Data</p><h1 class="text-2xl font-bold text-ink dark:text-white">Edit Guru</h1><p class="text-sm text-slate-500">Perbarui data guru.</p></div></template>
